...
import { htmlToLines, isMaterialLine, isVolatileLine, numericTokens, shape, stableLines, normalizeLine } from '../scripts/lib/text.mjs';
...
test('material lines: prices, quantities with units, free wording', () => {
  ...
  assert.ok(isMaterialLine('5.49 € per month'));
  assert.ok(isMaterialLine('ab 3,99 €'));
...
test('credential-shaped tokens are redacted before storage', () => {
  const a = normalizeLine('mysql://user:pscale_pw_AbCdEf123456@host/db');
  assert.ok(!a.includes('pscale_pw_') && a.includes('[redacted]'), a);
  const b = normalizeLine('Authorization: Bearer sk-abcdefghijklmnopqrstuvwxyz');
  assert.ok(!b.includes('sk-abc') && b.includes('[redacted]'), b);
  assert.equal(normalizeLine('$20 per month'), '$20 per month');
});
